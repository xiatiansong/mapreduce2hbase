...
public class IndexBuilder {
	...
	/** 建立索引的表名 **/
	private static final String  INDEX_TABLENAME = "index.tablename";
	/** 建立索引的列族名 **/
	private static final String  INDEX_FAMILYNAME = "index.familyname";
	/** 建立索引的列名 **/
	private static final String  INDEX_FIELDS = "index.fields";
	// 实现 Map 类
	public static class Map extends Mapper<ImmutableBytesWritable, Result, ImmutableBytesWritable, Writable> {
		...
		protected void setup(Context context) throws IOException, InterruptedException {
			...
			String tableName = conf.get(INDEX_TABLENAME);
			String[] fields = conf.getStrings(INDEX_FIELDS);
			// fields 内为需要做索引的列名
			String familyName = conf.get(INDEX_FAMILYNAME);
			family = Bytes.toBytes(familyName);
			// 初始化 indexes 方法
			indexes = new HashMap<byte[], ImmutableBytesWritable>();
			for (String field : fields) {
				// 如果给 name 做索引，则索引表的名称为“heroes‐name”
				indexes.put(Bytes.toBytes(field), new ImmutableBytesWritable(Bytes.toBytes(tableName + "_" + field)));
			}
		}
	}
	 ...
	 * @throws IOException
	 */
	private static void addRow(HTable table, String rowKey, String columnFamily, String column, String value) throws IOException {
	 ...
	 * @throws Exception
	 */
	public static void createIndexTable(Configuration conf, String tableName) throws Exception {
		// 新建一个数据库管理员
		HBaseAdmin hAdmin = new HBaseAdmin(conf);
		if (hAdmin.tableExists(tableName)) {
			System.out.println("该数据表已经存在，正在重新创建。");
	...
	public static void main(String[] args) throws Exception {
		...
		// 第二步：创建索引表
		for (String field : fields) {
			IndexBuilder.createIndexTable(conf, tableName + "_" + field);
		}
		// 第三步：进行 MapReduce 处理
		...
		// 设置传递属性值
		conf.set(INDEX_TABLENAME, tableName);
		conf.set(INDEX_FAMILYNAME, columnFamily);
		conf.setStrings(INDEX_FIELDS, fields);
